...
const CFG_BACKUP_SNIPPET = [
  ...
  "  // ALSO write the freshest snapshot to the FILESYSTEM (/data/config-backups/latest.json).",
  "  // ctx_backups lives in global context, so it is wiped along with everything else on a",
  "  // memory-only context, a full-deploy reset, or a container restart — which is exactly how",
  "  // a just-saved config (e.g. a new feed) could end up present as an engine tab but missing",
  "  // from the Configurator. The on-disk copy survives all of those, and is what Auto-Restore",
  "  // and Emergency Restore read back. Written synchronously so it is durable before the",
  "  // client triggers the engine-tab redeploy. (Fixed v0.9.50.)",
  "  try { var _fsB = global.get('fs'); if (_fsB) { _fsB.mkdirSync('/data/config-backups', { recursive: true }); _fsB.writeFileSync('/data/config-backups/latest.json', JSON.stringify(_snap, null, 2)); } } catch(_fe) { node.warn('latest.json write failed: '+_fe.message); }",
...
const configFlows = [
  ...
  },
  {
    ...
    func: [
      ...
      "} else {",
      "  var _doms = {};",
      "  (msg.payload.fields || []).forEach(function(f) {",
      "    if (f.domain && f.domain.type === 'codedValue' && f.domain.codedValues) {",
      "      var _m = {};",
      "      f.domain.codedValues.forEach(function(cv) { _m[cv.code] = cv.name; });",
      "      _doms[f.name] = _m;",
      "    }",
      "  });",
      "  msg.payload = {",
  ...
  },
  {
    ...
    func: [
      ...
      "  }",
      "  // Strip the localfilesystem/REST 'default' envelope so d holds clean top-level keys.",
      "  // The deploy-restore handler already does this; fn_cfg_restore historically did not,",
      "  // so a 'default'-wrapped backup read d.arcgis_configs === undefined and silently",
      "  // restored EMPTY — the 'backup browses but nothing loads' symptom. (Fixed v0.9.50.)",
      "  if (d && d['default'] && typeof d['default']==='object' && !Array.isArray(d['default'])) d = d['default'];",
      "  var ac=_pc(d.arcgis_configs); if(!Array.isArray(ac)) ac=[];",
      "  var tc=_pc(d.tc_configs);     if(!Array.isArray(tc)) tc=[];",
      ...
      "  // empty:true means the file parsed fine but held no configs (e.g. a backup taken",
      "  // AFTER a wipe) — the UI surfaces this so a no-op restore isn't read as success.",
      "  var _empty = (ac.length + tc.length + pp.length === 0) && Object.keys(ts).length === 0;",
      "  msg.payload = { ok:true, restored_from:filename, timestamp:d.timestamp, arcgis_count:ac.length, tc_count:tc.length, pp_count:pp.length, empty:_empty };",
      "} catch(e) {",
      "  msg.payload = { ok:false, error:e.message };",
      "}",
      "return msg;"
  ...
  },
  {
    ...
    func: [
      ...
      "  return true;",
      "}",
      "function _mergeArr(primary, secondary) {",
      "  // union by configName/name/id — primary (incoming backup) wins on conflict,",
      "  // secondary (current live) adds only names primary lacks. So a restore can NEVER",
      "  // DROP a live config the incoming backup happens to miss (e.g. one saved after the",
      "  // backup was captured). Both sources honor deletes, so this never resurrects either.",
      "  var out = [], seen = {};",
      "  function _k(c){ return (c && (c.configName || c.name || c.id)) || null; }",
      "  [primary, secondary].forEach(function(lst){",
      "    if (!Array.isArray(lst)) return;",
      "    lst.forEach(function(c){",
      "      var kk = _k(c);",
      "      if (kk === null) { out.push(c); return; }",
      "      if (seen[kk]) return; seen[kk] = 1; out.push(c);",
      "    });",
      "  });",
      "  return out;",
      "}",
      ...
      "    if (EXPECTED[k] === 'array') {",
      "      var _ex = global.get(k);",
      "      var _merged = _mergeArr(unwrapped, _typeOk(k, _ex) ? _ex : []);",
      "      global.set(k, _merged);",
      "      restored.push(k + '(' + _merged.length + (_merged.length > unwrapped.length ? '+keptlive' : '') + ')');",
      "    } else {",
  ...
  // ── One-time ghost cleanup (v10.1.5 W4): seed the tombstone ForceDelete sweeper ──
  {
    id: 'hi_ghostsweep', type: 'http in', z: CFG_TAB,
    name: 'POST /arcgis-tak/tak/seed-ghost-sweep',
    url: '/arcgis-tak/tak/seed-ghost-sweep', method: 'post',
    upload: false, swaggerDoc: '',
    x: 220, y: 1015, wires: [['fn_ghostsweep']]
  },
  {
    id: 'fn_ghostsweep', type: 'function', z: CFG_TAB,
    name: 'Queue ghost-sweep seed',
    func: [
      "var name = (msg.payload && msg.payload.configName) ? String(msg.payload.configName).trim() : '';",
      "if (!name) {",
      "  msg.statusCode = 400;",
      ...
      "  return msg;",
      "}",
      "var gs = global.get('_seedGhostSweep') || {};",
      "gs[name] = true;",
      "global.set('_seedGhostSweep', gs);",
      "var pollKey = '_lastPoll_' + name.replace(/[^A-Za-z0-9]/g, '_');",
      "global.set(pollKey, 0);",
      "node.warn('ghost-sweep seed queued for ' + name + ' — next poll reads the mission change log and ForceDelete-broadcasts every historical REMOVE uid for 14 days');",
      "msg.payload = { ok: true, queued: name, note: 'Next poll cycle seeds the tombstone sweeper from the mission change log. Devices clear ghosts as they connect over the next 14 days.' };",
      "return msg;"
    ].join('\n'),
    outputs: 1, timeout: '', noerr: 0,
    initialize: '', finalize: '', libs: [],
    x: 500, y: 1015, wires: [['ho_ghostsweep']]
  },
  {
    id: 'ho_ghostsweep', type: 'http response', z: CFG_TAB,
    name: '', statusCode: '', headers: {},
    x: 720, y: 1015, wires: []
  },
...
const FN_BUILD_QUERY = [
  ...
  "// v10.1.54 W1: the WHERE is built PER LAYER, not once for the whole config. Layers of one",
  "// service can carry different schemas: UASWFC_IR_Viewer uses PointDateTime / PolygonDateTime /",
  "// LineDateTime, and its IRLine layer has CreateDate AND DateCurrent 100% NULL across all 1,092",
  "// rows — so any single config-level timeField silently matches nothing on at least one layer,",
  "// which is indistinguishable from 'the source went empty'. Each layer entry may override",
  "// timeField / classField / where / timeFieldEpochMs; absent = fall back to the config-level",
  "// value, so every pre-10.1.54 config produces byte-identical queries.",
  "function buildWhere(c) {",
  "  var parts = [];",
  "  if (c.source.where) parts.push(c.source.where);",
  "  if (c.mapping.timeField && ttlMs(c) > 0) {",
  "    var cutoffMs = Date.now() - ttlMs(c);",
  "    if (c.mapping.timeFieldEpochMs) {",
  "      parts.push(c.mapping.timeField + ' >= ' + Math.floor(cutoffMs));",
  "    } else {",
  "      var cd = new Date(cutoffMs);",
  "      var y = cd.getUTCFullYear();",
  "      var mo = ('0' + (cd.getUTCMonth() + 1)).slice(-2);",
  "      var da = ('0' + cd.getUTCDate()).slice(-2);",
  "      parts.push(c.mapping.timeField + \" >= DATE '\" + y + '-' + mo + '-' + da + \"'\");",
  "    }",
  "  }",
  ...
  "  // v10.1.54 W1: per-layer overrides land on the clone BEFORE the WHERE is built, so the TTL",
  "  // clause uses THIS layer's time field. Everything downstream — dedup (mapping.timeField),",
  "  // the entity-id year suffix, class resolution (cfg.classField) and the style salt — already",
  "  // reads from this same clone, so none of it needs to change.",
  "  if (lyr.timeField) m._config.mapping.timeField = lyr.timeField;",
  "  if (lyr.dedupField) m._config.mapping.dedupField = lyr.dedupField;",
  "  if (lyr.dedupKeepAll != null) m._config.mapping.dedupKeepAll = lyr.dedupKeepAll;",
  "  if (lyr.classField) m._config.classField = lyr.classField;",
  "  if (lyr.where != null) m._config.source.where = lyr.where;",
  "  if (lyr.timeFieldEpochMs != null) m._config.mapping.timeFieldEpochMs = lyr.timeFieldEpochMs;",
  "  var where = buildWhere(m._config);",
  "  // The SAME layerId may now appear more than once with a different where/classField (e.g. one",
  "  // point layer split into heat-graded-by-intensity and markers-classed-by-category). layerName",
  "  // is what separates them in the UID layer tag, so duplicates must not collide. Only disambiguate",
  "  // when the id actually repeats — otherwise existing multi-layer feeds would churn every UID.",
  "  var _dupLayer = layers.filter(function(x) { return x.layerId === lyr.layerId; }).length > 1;",
  "  m._layerName = isMultiLayer ? (lyr.layerName || (_dupLayer ? String(lyr.layerId) + '_' + li : String(lyr.layerId))) : '';",
  "  // v10.1.31 W1: cache-bust. Public hosted feature layers are served through the ArcGIS",
  "  // Online CDN; FIRIS publishes CA_Perimeters_NIFC_FIRIS_public_view with cacheMaxAge=3600",
  "  // (Esri's 1h max) on data that updates several times an hour, so consecutive polls get",
  "  // contradictory answers for up to an hour after each flight. A unique param per request",
  "  // gives no cached object to serve -> TCP_MISS every time (verified live from CORAZ 2026-08-13;",
  "  // note cacheHint=false does NOT work, it just creates a second cache key). Costs 12",
  "  // origin requests/hour/feed. Esri doc: developers.arcgis.com .../feature-services/cdn-caching/",
  "  m.url = base + '/' + lyr.layerId + '/query'",
  "    + '?where=' + encodeURIComponent(where)",
  "    + '&outFields=*&returnGeometry=true&outSR=4326&f=json'",
  "    + '&_ts=' + Date.now() + '.' + li;",
  "  _whereLog.push((m._layerName || String(lyr.layerId)) + ': ' + where);",
  "  msgs.push(m);",
  "}",
  "node.warn(msg.topic + ' ArcGIS query (' + layers.length + ' layers) where: ' + _whereLog.join('   |   '));",
  "return [msgs];"
].join('\n');
...
const FN_PARSE_COT = [
  ...
  "  // v10.1.5 W3: deterministic dedup winner (PLAN-v10.1.5 §3b — BISCAR flip-flopped",
  "  // between competing rows every poll for hours, ~30 add/remove cycles amplifying",
  "  // EUD ghost churn). toMs parses once: numeric epoch passes through; date strings",
  "  // (e.g. '5/19/2026 11:22 AM') via Date. NaN = unparseable.",
  "  function toMs(v) {",
  ...
  "  }",
  "  // ArcGIS declares its real row-id field (objectIdFieldName in the query response).",
  "  // KML only carries a SYNTHETIC sequential OBJECTID (placemark index) — collapsing",
  "  // equal-timestamp records there would destroy multi-ring snapshots (FIRIS KML:",
  "  // every ring is its own placemark sharing source value + Created On), so the",
  "  // row-id tiebreak is ArcGIS-only; KML keeps ALL records at the winning timestamp.",
  "  var oidField = (msg.payload && msg.payload.objectIdFieldName) || null;",
  "  var groups = {};",
  "  var nBad = 0;",
  "  for (var di = 0; di < features.length; di++) {",
  "    var da = features[di].attributes || {};",
  "    // v10.1.54 W1: dedupField may be an ARRAY of fields, joined into one key. A single field",
  "    // is not always enough to identify 'the same thing republished': UASWFC_IR_Viewer reposts",
  "    // the NIFC daily perimeter per flight, but its polygon layer also carries IR Intense Heat",
  "    // and IR Scattered Heat for the same incident — deduping on IncidentName alone would keep",
  "    // only whichever category was published last and silently drop the others.",
  "    var key = Array.isArray(dedupField)",
  "      ? dedupField.map(function(fd) { return String(da[fd] == null ? '' : da[fd]); }).join('\\u0000')",
  "      : String(da[dedupField] || '');",
  "    var tms = toMs(da[timeField]);",
  "    if (isNaN(tms)) nBad++;",
  "    if (!groups[key]) groups[key] = [];",
  "    groups[key].push({ f: features[di], t: tms, o: oidField ? Number(da[oidField]) : NaN });",
  "  }",
  "  if (nBad > 0) node.warn(cfg.configName + ': dedup: ' + nBad + ' of ' + features.length + ' features have unparseable ' + timeField + ' — they lose to any parsed sibling');",
  "  var before = features.length;",
  "  features = [];",
  "  Object.keys(groups).sort().forEach(function(k) {",
  "    var recs = groups[k];",
  "    var parsed = recs.filter(function(r) { return !isNaN(r.t); });",
  "    var pool;",
  "    if (parsed.length === 0) {",
  "      // No usable timestamps: KML keeps all (never drop data); ArcGIS still collapses",
  "      // deterministically via the row-id tiebreak below (OIDs increment = latest proxy).",
  "      pool = recs;",
  "    } else {",
  "      var maxT = -Infinity;",
  "      parsed.forEach(function(r) { if (r.t > maxT) maxT = r.t; });",
  "      pool = parsed.filter(function(r) { return r.t === maxT; });",
  "    }",
  "    // Tiebreak 2 (ArcGIS-only): competing rows at the same timestamp -> highest row id wins.",
  "    // v10.1.54 W1: unless dedupKeepAll. The row-id collapse assumes one row IS one entity, which",
  "    // is true for a perimeter but false for any MULTI-FEATURE product: UASWFC_IR_Viewer publishes",
  "    // a mop-up buffer as ~30 separate line segments that all share one LineDateTime, so collapsing",
  "    // on row id would keep ONE segment and silently discard the other 29. dedupKeepAll keeps every",
  "    // row at the winning timestamp and drops only the older snapshots — the same rule KML already",
  "    // gets for free above, now selectable per layer.",
  "    var keepAll = !!(cfg.mapping && cfg.mapping.dedupKeepAll);",
  "    if (pool.length > 1 && oidField && !keepAll) {",
  "      var withOid = pool.filter(function(r) { return !isNaN(r.o); });",
  "      if (withOid.length > 0) {",
  "        var maxO = -Infinity;",
  "        withOid.forEach(function(r) { if (r.o > maxO) maxO = r.o; });",
  "        pool = withOid.filter(function(r) { return r.o === maxO; });",
  "      }",
  "      // Tiebreak 3: pure-determinism guard — lexicographic on stringified attributes.",
  "      if (pool.length > 1) {",
  "        pool.sort(function(a, b) {",
  "          var sa = JSON.stringify(a.f.attributes || {}), sb = JSON.stringify(b.f.attributes || {});",
  "          return sa < sb ? -1 : (sa > sb ? 1 : 0);",
  "        });",
  "        pool = [pool[0]];",
  "      }",
  "    }",
  "    pool.forEach(function(r) { features.push(r.f); });",
  "  });",
  "  if (features.length < before) {",
  "    node.warn(cfg.configName + ': dedup by ' + (Array.isArray(dedupField) ? dedupField.join('+') : dedupField) + ': ' + before + ' -> ' + features.length + ' features');",
  "  }",
  "}",
  "",
  FN_TTL,
  "",
  "function _assignDetail(t, src) { for (var _k in src) if (Object.prototype.hasOwnProperty.call(src, _k)) t[_k] = src[_k]; return t; }",
  ...
  "}",
  "",
  "// --- Douglas-Peucker geometry simplification (dependency-free) ---",
  "// Dense source polygons (e.g. nationwide NWS warning zones, 1000s of verts each)",
  "// produce a multi-MB CoT mission snapshot that stalls CloudTAK's all-at-once",
  ...
  "// Per-polygon POINT CAP, auto-tuned from the feed's feature COUNT (deterministic fleet rule, same",
  "// on every box). Few features -> generous cap (a big detailed fire perimeter or NWS zone keeps its",
  "// shape); many features -> small cap (e.g. WWA's 671 polys stay light so the CloudTAK initial",
  ...
  "  if (len2 === 0) { var ex0=p[0]-a[0], ey0=p[1]-a[1]; return Math.sqrt(ex0*ex0+ey0*ey0); }",
  ...
  "    var maxD = -1, idx = -1;",
  "    for (var di = s+1; di < e; di++) {",
  "      var d = _perpDist(pts[di], pts[s], pts[e]);",
  "      if (d > maxD) { maxD = d; idx = di; }",
  "    }",
  "    if (maxD > tol && idx > -1) { keep[idx] = true; stack.push([s, idx]); stack.push([idx, e]); }",
  "  }",
  "  var out = [];",
  "  for (var ki = 0; ki < n; ki++) if (keep[ki]) out.push(pts[ki]);",
  "  return out;",
  "}",
  "// v10.1.54 W1: a flat cap is blind to how big the geometry is. Rowe Creek's mop-up buffer is a",
  "// 731 km line of 176,236 verts — the 400 cap kept 0.2% of it, while the small spot-fire buffers",
  "// (<400 verts) passed through whole. Hence 'the small ones are detailed and the big one is obtuse'.",
  "// Scale the cap with the geometry's own size, floored at _PT_CAP so nothing gets WORSE than today",
  "// and ceilinged so one huge shape can't blow up the CoT.",
  "function _capFor(n) { return Math.min(_GEOM_CEIL, Math.max(_PT_CAP, Math.ceil(n / 8))); }",
  "function _simp(pts, isRing, cap) {",
  "  cap = cap || _PT_CAP;",
  "  if (!pts || pts.length <= cap) return pts;",
  "  // Over cap: binary-search the DP tolerance so the result lands JUST under the cap (uses the full",
  "  // budget — a plain escalate overshoots and lands far under, leaving big shapes too coarse).",
  "  var lo = 0, hi = 1, out = _dp(pts, hi);",
  "  for (var _bs = 0; _bs < 16; _bs++) {",
  "    var mid = (lo + hi) / 2, r = _dp(pts, mid);",
  "    if (r.length > cap) { lo = mid; } else { hi = mid; out = r; }",
  "  }",
  "  if (isRing) {",
  "    if (out.length < 4) return pts;",
  "    var f = out[0], l = out[out.length-1];",
  "    if (f[0] !== l[0] || f[1] !== l[1]) out.push([f[0], f[1]]);",
  "  } else if (out.length < 2) { return pts; }",
  "  return out;",
  "}",
  "",
  "var results = [];",
  ...
  "  var f = features[i];",
  "  var a = f.attributes || {};",
  "  if (cfg.domains) { for (var _df in cfg.domains) { if (a[_df] != null) { var _dm = cfg.domains[_df]; if (_dm && _dm[a[_df]] != null) a[_df] = _dm[a[_df]]; } } }",
  "  var g = f.geometry;",
  "  if (!g) continue;",
  "  var gKey;",
...
"  hp.push('v' + _COT_FMT + 'g' + _PT_CAP + 'b' + Math.min(_GEOM_CEIL, _PT_CAP * 8) + 'm' + Number((cfg.mapping && cfg.mapping.minRingVerts) || 0));",
"  hp.push('s' + _STYLE_SALT);",
"  var _hash = djb2(hp.join('|'));",
  ...
  "  }",
  "  // v10.1.5 W2: optional stable per-entity UID (PLAN §3a — snapshot sources mint a",
  "  // NEW row per update, so row-keyed UIDs turn every repost into DELETE(old)+ADD(new)",
  "  // and any EUD offline at the REMOVE keeps the old polygon forever). When",
  "  // mapping.entityIdField is set and the value is non-blank, the UID follows the",
  "  // entity value plus the feature's timeField YEAR (cross-season name reuse must not",
  "  // resurrect a prior year's UID). Blank/missing value or no entityIdField -> the",
  "  // row-ID path above is used unchanged (outage-style feeds untouched).",
  "  var entityIdField = (cfg.mapping && cfg.mapping.entityIdField && String(cfg.mapping.entityIdField).trim()) || null;",
  "  if (entityIdField && a[entityIdField] != null && String(a[entityIdField]).trim() !== '') {",
  "    idVal = String(a[entityIdField]).trim();",
  "    var _eT = NaN;",
  "    if (timeField && a[timeField] != null && a[timeField] !== '') {",
  "      var _eN = Number(a[timeField]);",
  "      _eT = !isNaN(_eN) ? _eN : new Date(String(a[timeField])).getTime();",
  "    }",
  "    if (!isNaN(_eT) && _eT > 0) idVal += '-' + new Date(_eT).getUTCFullYear();",
  "  }",
  ...
  "  var fColor_s = noColor ? null : (cls.strokeColor || cls.color || sColor);",
  "  var fColor_f = noColor ? null : (cls.fillColor || cls.color || fColor);",
  "  var fClsAlpha = (cls.fillAlpha != null && cls.fillAlpha !== '') ? Number(cls.fillAlpha) / 100 : fillAlphaFloat;",
  "  var fClsWeight = (cls.strokeWeight != null && cls.strokeWeight !== '') ? Number(cls.strokeWeight) : (cfg.style.strokeWeight || 3);",
  "  var fClsStyle = cls.strokeStyle || cfg.style.strokeStyle || 'solid';",
  "  var fStrokeArgb = fColor_s ? hexArgb(fColor_s, 1) : null;",
  "  var fFillArgb = fColor_f ? hexArgb(fColor_f, fClsAlpha) : null;",
  "",
  "  var _parts = [];",
  "  if (kind === 'polygon') {",
  "    var _outer = [];",
  ...
  "    // v10.1.54 W1: a MULTIPOLYGON's vertex budget belongs to the FEATURE, not to each ring.",
  "    // _PT_CAP is a flat per-ring cap tuned on FIRIS/NWS, where a 'big detailed perimeter' meant",
  "    // ~1-2k verts. UASWFC's Rowe Creek perimeter is ONE feature of 194 rings / 68,956 verts, of",
  "    // which a single ring holds 59,924 — the cap crushed that one ring 150:1 to 400 points while",
  "    // leaving 186 sub-cap specks (median 8 verts) untouched. Exactly backwards. Allocate the",
  "    // budget in proportion to each ring's size so the dominant ring keeps its shape and the",
  "    // confetti still costs almost nothing. Single-ring features are UNCHANGED (one ring gets the",
  "    // whole _PT_CAP), so every existing single-ring feed produces byte-identical geometry.",
  "    var _ringCaps = null;",
  "    if (_outer.length > 1) {",
  "      var _totV = 0;",
  "      for (var _rc = 0; _rc < _outer.length; _rc++) _totV += _outer[_rc].length;",
  "      var _budget = Math.min(_GEOM_CEIL, _PT_CAP * 8);",
  "      _ringCaps = [];",
  "      for (var _rc2 = 0; _rc2 < _outer.length; _rc2++) {",
  "        var _share = _totV > 0 ? (_outer[_rc2].length / _totV) : 0;",
  "        _ringCaps.push(Math.max(6, Math.round(_budget * _share)));",
  "      }",
  "    }",
  "    // Opt-in speck filter. A ring of a handful of verts on a 355k-acre perimeter costs a whole CoT,",
  "    // a UID and a map label to draw something invisible — and each is named after its",
  "    // FeatureCategory, so 54 of them read as 54 separate 'IR Heat Perimeter' shapes sitting on top",
  "    // of the heat points. OFF unless minRingVerts is set: on a heat feed a small ring can be a real",
  "    // small heat area, and silently dropping fire data is not a default we get to choose.",
  "    var _minRV = Number((cfg.mapping && cfg.mapping.minRingVerts) || 0);",
  "    if (_minRV > 0 && _outer.length > 1) {",
  "      var _kept = [], _keptCaps = [];",
  "      for (var _rk = 0; _rk < _outer.length; _rk++) {",
  "        if (_outer[_rk].length >= _minRV) { _kept.push(_outer[_rk]); if (_ringCaps) _keptCaps.push(_ringCaps[_rk]); }",
  "      }",
  "      if (_kept.length) {",
  "        if (_kept.length < _outer.length) node.warn(cfg.configName + ': dropped ' + (_outer.length - _kept.length) + ' of ' + _outer.length + ' ring(s) under ' + _minRV + ' verts');",
  "        _outer = _kept; if (_ringCaps) _ringCaps = _keptCaps;",
  "      }",
  "    }",
  ...
  "      _parts.push({ uid: _isMultiPoly ? uid+'-r'+_ri2 : uid, _hash: _isMultiPoly ? djb2(_hash+'|r'+_ri2+'|'+_rgk) : _hash, lat: _rsy/_rr2.length, lon: _rsx/_rr2.length, verts: _simp(_rr2, true, _ringCaps ? _ringCaps[_ri2] : _capFor(_rr2.length)) });",
  "    }",
  ...
  "      _parts.push({ uid: _isMultiPath ? uid+'-p'+_pi : uid, _hash: _isMultiPath ? djb2(_hash+'|p'+_pi+'|'+_pgk) : _hash, lat: _psy/_pp.length, lon: _psx/_pp.length, verts: _simp(_pp, false, _capFor(_pp.length)) });",
  "    }",
  "  } else {",
  ...
  "  } else if (cfg.remarksTemplate && String(cfg.remarksTemplate).trim()) {",
  "    // v10.1.54 W1: the CONFIG-level remarksTemplate was written by the configurator and never",
  "    // read here — the chain went class-template -> class-fields -> config-FIELDS, skipping",
  "    // config-TEMPLATE. Any feed that set a template without also setting remarksFields, or",
  "    // without a per-class template on every class, silently shipped empty remarks. Mirrors the",
  "    // callsign precedence above, which does honour cfg.style.labelTemplate.",
  "    remarks = tmpl(cfg.remarksTemplate, a);",
  "  } else {",
  ...
  "  }",
  "",
  "  // v10.1.54 W1: style.centerLabel / class.centerLabel — 'Center label in shape'. The configurator",
  "  // has shipped this dropdown since forever and the engine has NEVER read it (7 refs in",
  "  // configurator.html, 0 here), so ticking it did nothing. ATAK draws a drawn-shape's CENTRE",
  "  // MARKER and labels it from <contact callsign> regardless of labels_on — which is why a",
  "  // perimeter with labels_on=false still puts a named dot in the middle of the map. The only",
  "  // lever is the callsign itself, so drop <contact> on non-point geometry when it is off.",
  "  // Trade-off, deliberately not hidden: the shape then has no name in ATAK's overlay manager.",
  "  // v10.1.54 W1: centerLabel maps to ATAK's <__shapeExtras cpvis> — 'centre point visible'.",
  "  // Captured by having the operator draw two identical shapes with ATAK's Center toggle on and",
  "  // off and send both to the mission: the CoTs differ in exactly one attribute,",
  "  //   ON  -> <__shapeExtras cpvis=\"true\"  editable=\"true\"/>",
  "  //   OFF -> <__shapeExtras cpvis=\"false\" editable=\"true\"/>",
  "  // It has nothing to do with the callsign — an earlier attempt dropped <contact>, which removed",
  "  // the centre marker's LABEL and left the marker, and cost the shape its name in the overlay",
  "  // manager for nothing. Shapes keep their contact.",
  "  var _ctrPt = (cls.centerLabel != null) ? cls.centerLabel : (cfg.style.centerLabel != null ? cfg.style.centerLabel : true);",
  "  var detail = {};",
  "  detail.contact = [{ _attributes: { callsign: callsign } }];",
  "  if (kind !== 'point') detail.__shapeExtras = [{ _attributes: { cpvis: (_ctrPt === false ? 'false' : 'true'), editable: 'true' } }];",
  "  _assignDetail(detail, {",
  "    remarks: remarks,",
  "    labels_on: [{ _attributes: { value: (cls.labelsOn != null ? cls.labelsOn : cfg.style.labelsOn) ? 'true' : 'false' } }]",
  "  });",
  "  // v10.1.54 W1: strokeColor / fillColor / strokeWeight are SHAPE details and must not ride on a",
  "  // marker. strokeStyle was already point-excluded below — the same reasoning applies to the rest,",
  "  // it was just never followed through, and no point feed existed to expose it. Sending shape",
  "  // styling on a b-m-p-s-m appears to make ATAK render it as a shape and ignore <color argb>,",
  "  // which is why every spot map came out white even with the correct colour on the wire.",
  "  if (fStrokeArgb != null && kind !== 'point') detail.strokeColor = [{ _attributes: { value: String(fStrokeArgb) } }];",
  "  // v10.1.54 W1: colour the MARKER. strokeColor/fillColor style a drawn shape; ATAK colours a",
  "  // point from <color argb>, and with no <color> a marker falls back to its default rendering —",
  "  // which for a spot-map point (b-m-p-s-m) is WHITE. Every ArcGIS feed before this one was",
  "  // polygons (CA AIR INTEL, FIRIS, POWER-OUTAGES, NWS zones), so no point feed had ever been",
  "  // asked to carry a class colour and the omission never showed. Full alpha: a marker is not a fill.",
  "  if (kind === 'point' && fStrokeArgb != null) detail.color = [{ _attributes: { argb: String(fStrokeArgb) } }];",
  "  if (!noColor && kind !== 'point') detail.strokeWeight = [{ _attributes: { value: String(fClsWeight) + '.0' } }];",
  "  if (!noColor && kind !== 'point') detail.strokeStyle = [{ _attributes: { value: String(fClsStyle) } }];",
  "  if (fFillArgb != null && kind !== 'point') detail.fillColor = [{ _attributes: { value: String(fFillArgb) } }];",
  "",
  "  // v10.1.54 W1: a Spot Map marker is coloured by its ICON PATH, not by <color argb>. Captured",
  "  // from a real ATAK device via /Marti/api/missions/<name>/changes — a spot map ATAK itself",
  "  // created carries iconsetPath 'COT_MAPPING_SPOTMAP/b-m-p-s-m/-256' alongside color '-256'.",
  "  // <color> only TINTS the icon; with no usericon there is nothing to tint and ATAK draws its",
  "  // default white marker. That is why every spot map came out white while the colour was",
  "  // provably correct on the wire and correctly indexed by TAK. Synthesize the path from the",
  "  // resolved colour unless the class supplies its own icon.",
  "  if (!fIconsetpath && kind === 'point' && fCotType === 'b-m-p-s-m' && fStrokeArgb != null) {",
  "    fIconsetpath = 'COT_MAPPING_SPOTMAP/b-m-p-s-m/' + String(fStrokeArgb);",
  "  }",
  ...
  "  if (verts) {",
  "    var links = [];",
  "    // verts is already capped to _PT_BUDGET by _simp() — just emit the link points.",
  "    for (var j=0;j<verts.length;j++) links.push({ _attributes: { point: verts[j][1]+','+verts[j][0] } });",
  "    detail.link = links;",
  "  }",
  "",
  "    results.push({",
...
const FN_COT_TO_XML = [
  "// v10.1.5 W4: pre-built XML (tombstone t-x-d-d ForceDelete) passes straight through.",
  "if (msg._rawCotXml) { msg.payload = Buffer.from(msg._rawCotXml, 'utf8'); return msg; }",
  ...
  "}",
  "",
  "// v10.1.54 W1: <color argb> — a MARKER's colour. cot_to_xml builds from an explicit whitelist of",
  "// detail keys, and 'color' was never one of them, so parse_cot's detail.color was generated and",
  "// silently dropped here. That is why spot maps stayed white through three attempts: the colour",
  "// was correct in the CoT JSON at every step and never reached the wire. ATAK pairs it with the",
  "// usericon path (captured from a device: <color argb=\"-16711681\"/> next to",
  "// iconsetpath .../-16711681), so emit it immediately before usericon to match.",
  "// v10.1.54 W1: __shapeExtras carries cpvis (centre point visible). This converter is a",
  "// WHITELIST — see the <color> bug — so a new detail key MUST have a case here or it is",
  "// generated and silently dropped before the wire.",
  "if (d.__shapeExtras && d.__shapeExtras[0]) xml += '<__shapeExtras cpvis=\"' + d.__shapeExtras[0]._attributes.cpvis + '\" editable=\"' + d.__shapeExtras[0]._attributes.editable + '\"/>';",
  "if (d.color && d.color[0]) xml += '<color argb=\"' + d.color[0]._attributes.argb + '\"/>';",
...
function makeEngineTab(feed) {
  ...
  const FN_RECONCILE = [
    ...
    "var newHashes = {};",
    "var nStream = 0, nSkip = 0, nPut = 0, nDel = 0, nSeed = 0, nRefresh = 0;",
    "var newUids = [];",
    "",
    "// v10.1.34: keepalive re-stream. We only stream a feature when it is NEW or CHANGED,",
    "// which is the right call for bandwidth but rots quiet feeds: TAK ages the CoT out of",
    "// its repository while the mission keeps the UID, leaving the mission pointing at shapes",
    "// that no longer exist. Measured on test12 2026-08-13 — POWER-OUTAGES had 273 uids and",
    "// 0 events from GET /Marti/api/missions/<n>/cot, against a healthy CA AIR INTEL at 80/80.",
    "// Symptom is ATAK sitting on the yellow pending-download icon forever; it only bites",
    "// clients that subscribe or re-subscribe, so it stays invisible until someone new joins.",
    "// Perverse property: the QUIETER and better-behaved a feed is, the more likely it breaks.",
    "// So re-stream every UID at least once per REFRESH_MS, spread across polls so a big feed",
    "// does not re-send everything in one burst.",
    "var REFRESH_MS = 6 * 3600000;   // 6h — comfortably inside TAK's CoT retention",
    "var REFRESH_MAX_PER_POLL = 25;  // spread the work; a 273-uid feed refreshes over ~11 polls",
    "var seenKey = '_lastStreamed' + (msg._layerPrefix ? '_' + msg._layerPrefix : '');",
    "var lastStreamed = flow.get(seenKey) || {};",
    "var _nowStream = Date.now();",
    "",
    ...
    "  // Due for a keepalive? Only for UIDs the mission already holds (a new one is being",
    "  // streamed anyway) and only up to the per-poll cap.",
    "  var _due = (!changed && existing[uid] && !coldStart",
    "              && nRefresh < REFRESH_MAX_PER_POLL",
    "              && (_nowStream - (lastStreamed[uid] || 0)) > REFRESH_MS);",
    "  if (coldStart && existing[uid]) {",
    "    nSeed++;",
    "    lastStreamed[uid] = _nowStream;  // seeded, not streamed — start its clock now",
    "  } else if (changed || !existing[uid] || _due) {",
    "    if (_due) { nRefresh++; }",
    "    lastStreamed[uid] = _nowStream;",
    "    nStream++;",
    "    node.send([",
    ...
    "}",
    "// v10.1.34: prune keepalive timestamps for UIDs the source no longer returns, so the",
    "// map cannot grow without bound on a feed with churning UIDs.",
    "for (var _lsu in lastStreamed) { if (!arcgis[_lsu]) delete lastStreamed[_lsu]; }",
    "flow.set(seenKey, lastStreamed);",
    "if (nRefresh > 0) node.warn(topicCfg + ': keepalive re-streamed ' + nRefresh + ' unchanged UID(s) so their CoT cannot age out of the TAK repository');",
    ...
    "}",
    "",
    "// v10.1.5 W4: tombstone map — every reconcile DELETE is recorded and re-broadcast",
    "// as t-x-d-d + <__forcedelete/> (ATAK CotDeleteImporter's unconditional removal)",
    "// on the same stream connection the feed's CoT rides, every poll for 14 days.",
    "// ATAK never reaps stale shapes (CotMarkerRefresher iterates markers only) and",
    "// resync never replays missed REMOVE_CONTENTs, so an EUD offline at DELETE keeps",
    "// the polygon forever without this. Volatile flow context BY DESIGN: loss on",
    "// restart only means less retroactive cleanup, never data loss.",
    "// v10.1.34: the sweep is capped three ways. Measured on test12 2026-08-13: ~11.9k",
    "// ForceDelete CoT every 5 minutes across three feeds (POWER-OUTAGES 11,397 +",
    "// CA AIR INTEL 433 + NWS 27) — roughly 38/sec, sustained, to EVERY connected EUD",
    "// including anything on cellular. Every one of those broadcasts was CORRECT (outages",
    "// genuinely resolve at ~800/day and the TTL pruned properly); it is a volume problem,",
    "// not a correctness one. Three independent reductions:",
    "//   TTL      14d -> 3d. Most EUDs reconnect within a day or two; 14d was generous and",
    "//            it is dwarfed by the feeds' own retention (CA AIR INTEL keeps 72h of data",
    "//            but was nagging about removals for two weeks).",
    "//   BURST    cap per poll, oldest-first, so a huge backlog drains steadily instead of",
    "//            spiking the stream every 5 minutes.",
    "//   RETRIES  a tombstone broadcast MAX_SENDS times has done its job; a device that has",
    "//            not reconnected in that many polls will resync from the mission anyway.",
    "var TOMB_TTL = 3 * 24 * 3600000;",
    "var TOMB_MAX_PER_SWEEP = 250;",
    "var TOMB_MAX_SENDS = 48;",
    "var tombKey = '_tombstones';",
    "var tombSendsKey = '_tombSends';",
    "var tombs = flow.get(tombKey) || {};",
    "var tombSends = flow.get(tombSendsKey) || {};",
    "",
    "// v10.1.31 W2: delete hysteresis. A UID must be absent from MISS_THRESHOLD consecutive",
    "// polls before it is DELETEd or tombstoned. W1 is the primary fix (we no longer get served",
    "// stale CDN copies); this is the backstop for a genuine upstream/network blip. Fleet",
    "// constant, deliberately NOT a per-config knob.",
    "// v10.1.34: TIME-based, not poll-count based. The count version shipped as 3 polls —",
    "// 15 min at the 5-min default, but only 3 min on a 1-min feed. The TFR engine lost 16 of",
    "// 17 live shapes to a 4-minute upstream blip on test12 2026-08-15 for exactly that reason.",
    "// The guarantee must be 'absent for N MINUTES', independent of poll cadence.",
    "var MIN_ABSENCE_MS = 15 * 60000;",
    "var MASS_ABSENCE_MS = 30 * 60000;",
    "var missKey = '_missStreak';",
    "var misses = flow.get(missKey) || {};",
    "var nHeld = 0;",
    "var _nowMiss = Date.now();",
    "// Migrate v10.1.31 state: those values were small counters, these are epoch ms. A stale",
    "// counter read as a timestamp is ~1970 and would delete instantly on the next poll.",
    "for (var _mk in misses) { if (!(misses[_mk] > 1e12)) misses[_mk] = _nowMiss; }",
    "",
    "// v10.1.5 W4: one-time fleet ghost cleanup — operator queued a change-log seed via",
    "// the Configurator; pull every historical REMOVE_CONTENT uid into the tombstone map",
    "// so the sweeper clears ghosts off every device that connects within the window.",
    "var seedQ = global.get('_seedGhostSweep') || {};",
    "if (seedQ[topicCfg] === true) {",
    "  delete seedQ[topicCfg];",
    "  global.set('_seedGhostSweep', seedQ);",
    "  try {",
    "    var _sc, _sk;",
    "    if (_nodeFs.existsSync('/certs/nodered.pem') && _nodeFs.existsSync('/certs/nodered.key')) { _sc = '/certs/nodered.pem'; _sk = '/certs/nodered.key'; }",
    "    else { _sc = '/certs/admin.pem'; _sk = '/certs/admin.key'; }",
    "    var _sco = _nodeFs.existsSync(_sc) ? { cert: _nodeFs.readFileSync(_sc), key: _nodeFs.readFileSync(_sk), passphrase: 'atakatak' } : {};",
    "    var _sro = Object.assign({ hostname: host, port: parseInt(String(tak.missionApiPort || 8443)),",
    "      path: '/Marti/api/missions/' + encodeURIComponent(cfg.missionName) + '/changes?squashed=false',",
    "      method: 'GET', rejectUnauthorized: false, headers: { 'accept': 'application/json' } }, _sco);",
    "    if (bearer) _sro.headers.Authorization = 'Bearer ' + bearer;",
    "    else if (cookie) _sro.headers.Cookie = cookie;",
    "    var _sReq = _nodeHttps.request(_sro, function(_rs) {",
    "      var _bufs = [];",
    "      _rs.on('data', function(d) { _bufs.push(d); });",
    "      _rs.on('end', function() {",
    "        try {",
    "          var _cj = JSON.parse(Buffer.concat(_bufs).toString('utf8'));",
    "          var _chs = (_cj && _cj.data) || [];",
    "          var _t2 = flow.get(tombKey) || {};",
    "          var _nSeed = 0;",
    "          for (var _ci = 0; _ci < _chs.length; _ci++) {",
    "            var _ch = _chs[_ci];",
    "            if (!_ch || _ch.type !== 'REMOVE_CONTENT') continue;",
    "            var _cu = _ch.contentUid || (_ch.details && _ch.details.uid) || null;",
    "            if (!_cu || existing[_cu] || arcgis[_cu] || _t2[_cu]) continue;",
    "            _t2[_cu] = Date.now(); _nSeed++;",
    "          }",
    "          flow.set(tombKey, _t2);",
    "          node.warn(topicCfg + ' ghost-sweep seed: ' + _nSeed + ' historical REMOVE uids queued for ForceDelete (HTTP ' + _rs.statusCode + ')');",
    "        } catch(e) { node.warn(topicCfg + ' ghost-sweep seed parse error: ' + e.message); }",
    "      });",
    "    });",
    "    _sReq.on('error', function(e) { node.warn(topicCfg + ' ghost-sweep seed error: ' + e.message); });",
    "    _sReq.end();",
    "  } catch(e) { node.warn(topicCfg + ' ghost-sweep seed setup error: ' + e.message); }",
    "}",
    "",
    "if (!arcgisOk) {",
    ...
    "  }",
    "  // v10.1.31 W4: magnitude guard. A single poll that claims most of the mission just",
    "  // vanished is a bad answer, not an event — a stale CDN copy, a truncated response, or a",
    "  // config change that narrowed the query (test12 2026-08-13: a TTL cut from 30d to 72h",
    "  // deleted 148 of 148 shapes three minutes later). Require the claim to repeat across",
    "  // MISS_THRESHOLD polls before acting. An explicit one-shot purge still overrides.",
    "  // A mass claim must persist LONGER than an ordinary one — that is the whole difference",
    "  // between W4 and W2. It still converges, so a feed that genuinely empties out is cleaned",
    "  // up; it just takes MASS_MISS_THRESHOLD polls instead of MISS_THRESHOLD.",
    "  var MAG_FRACTION = 0.25;",
    "  var MAG_FLOOR = 5;  // tiny missions must stay able to converge",
    "  var candidates = [];",
    "  for (var uid in existing) {",
    "    var matchesPrefix = (uid.indexOf(prefix) === 0);",
    "    if (!arcgis[uid] && (cleanOrphans || matchesPrefix)) candidates.push(uid);",
    "  }",
    "  var nExisting = Object.keys(existing).length;",
    "  var massDelete = !oneShotPurge && candidates.length > MAG_FLOOR",
    "    && candidates.length > nExisting * MAG_FRACTION;",
    "  var needAbsent = massDelete ? MASS_ABSENCE_MS : MIN_ABSENCE_MS;",
    "",
    "  for (var ci = 0; ci < candidates.length; ci++) {",
    "    var duid = candidates[ci];",
    "    if (!misses[duid]) misses[duid] = _nowMiss;   // first poll it went missing",
    "    // W2 hysteresis (+ W4's longer bar when the whole pass looks like a mass delete).",
    "    if (!oneShotPurge && (_nowMiss - misses[duid]) < needAbsent) { nHeld++; continue; }",
    "    delete misses[duid];",
    "    nDel++;",
    "    tombs[duid] = Date.now();  // v10.1.5 W4: swept as ForceDelete below + every poll for 14d",
    "    node.send([null, {",
    "      method: 'DELETE',",
    "      url: baseUrl + '/contents?uid=' + encodeURIComponent(duid) + '&creatorUid=' + creator,",
    "      headers: { 'accept': '*/*' },",
    "      _missionCookie: cookie,",
    "      _missionBearer: bearer,",
    "      payload: '',",
    ...
    "      + ' of ' + nExisting + ' mission UIDs are gone; holding ' + nHeld",
    "      + ' until they have been absent ' + (MASS_ABSENCE_MS / 60000) + ' min');",
    "  }",
    "  // Any UID the source still returns clears its streak — a single good poll resets it.",
    "  for (var au in arcgis) { if (misses[au]) delete misses[au]; }",
    "  // Prune streaks for UIDs no longer in the mission at all, so the map cannot grow unbounded.",
    "  for (var mu in misses) { if (!existing[mu]) delete misses[mu]; }",
    "  if (oneShotPurge) {",
    ...
    "}",
    "",
    "// v10.1.5 W4: sweep — prune expired tombstones, then broadcast t-x-d-d ForceDelete",
    "// for the live ones on the feed's stream connection (bare broadcast, NO <marti dest>:",
    "// the mission-dest path was observed not to deliver task CoT, 2026-07-20). At most",
    "// once per 45s per flow so multi-layer passes within one poll don't triple-send.",
    "var _now2 = Date.now();",
    "var _lastSweep = flow.get('_tombSweepTs') || 0;",
    "if (_now2 - _lastSweep >= 45000) {",
    "  var sweepUids = [];",
    "  var nRetired = 0;",
    "  for (var tu in tombs) {",
    "    if (_now2 - tombs[tu] > TOMB_TTL) { delete tombs[tu]; delete tombSends[tu]; continue; }",
    "    // v10.1.31 W3: 'came back' must include the mission's own contents, not just this",
    "    // poll's ArcGIS result. Checking arcgis alone let us ForceDelete a UID off every EUD",
    "    // while the mission still held it (CORAZ 2026-08-13, and any multi-layer pass where",
    "    // arcgis carries only one layer's UIDs). This is the last line of defence.",
    "    if (arcgis[tu] || existing[tu]) { delete tombs[tu]; delete tombSends[tu]; continue; }  // never kill a live shape",
    "    // v10.1.34: retire by send-count. Broadcasting the same removal 4,000 times is waste,",
    "    // not safety — a device that has missed 48 consecutive sweeps is not coming back to",
    "    // this stream mid-window, and resyncs from the mission when it does.",
    "    if ((tombSends[tu] || 0) >= TOMB_MAX_SENDS) { nRetired++; continue; }",
    "    sweepUids.push(tu);",
    "  }",
    "  // v10.1.34: oldest-first, then cap the burst. Fresh removals matter most (a device",
    "  // still holding a just-deleted shape), and draining steadily beats a 5-minute spike.",
    "  sweepUids.sort(function(a, b) { return (tombs[a] || 0) - (tombs[b] || 0); });",
    "  var _sweepTotal = sweepUids.length;",
    "  if (sweepUids.length > TOMB_MAX_PER_SWEEP) sweepUids = sweepUids.slice(0, TOMB_MAX_PER_SWEEP);",
    "  if (sweepUids.length > 0) {",
    "    flow.set('_tombSweepTs', _now2);",
    "    var _tIso = new Date(_now2).toISOString();",
    "    var _tStale = new Date(_now2 + 300000).toISOString();",
    "    var _sport = Number(cfg.cotStreamPort) || Number(cfg.streamPort) || Number(tak.streamingPort) || Number(tak.streamPort) || 8089;",
    "    var _xesc = function(s) { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/\"/g,'&quot;'); };",
    "    for (var si = 0; si < sweepUids.length; si++) {",
    "      var _su = _xesc(sweepUids[si]);",
    "      var _fx = '<event version=\"2.0\" uid=\"fd.' + _su + '\" type=\"t-x-d-d\" how=\"m-g\"'",
    "        + ' time=\"' + _tIso + '\" start=\"' + _tIso + '\" stale=\"' + _tStale + '\">'",
    "        + '<point lat=\"0\" lon=\"0\" hae=\"9999999\" ce=\"9999999\" le=\"9999999\"/>'",
    "        + '<detail><link uid=\"' + _su + '\" relation=\"none\" type=\"none\"/><__forcedelete/></detail></event>\\n';",
    "      node.send([{ _rawCotXml: _fx, topic: topicCfg, host: host, port: _sport }, null]);",
    "      tombSends[sweepUids[si]] = (tombSends[sweepUids[si]] || 0) + 1;",
    "    }",
    "    node.warn(topicCfg + ' tombstone sweep: ' + sweepUids.length + ' ForceDelete broadcast(s)'",
    "      + (_sweepTotal > sweepUids.length ? ' (capped from ' + _sweepTotal + ' due this poll — draining ' + TOMB_MAX_PER_SWEEP + '/poll)' : '')",
    "      + (nRetired > 0 ? ', ' + nRetired + ' retired after ' + TOMB_MAX_SENDS + ' sends' : ''));",
    "  }",
    "}",
    "flow.set(tombKey, tombs);",
    "flow.set(tombSendsKey, tombSends);  // v10.1.34: send-count retirement must persist",
    "flow.set(missKey, misses);  // v10.1.31 W2: streak map must survive to the next poll",
    "",
  ...
  return [
    ...
    },
    {
      ...
      func: [
        ...
        "}",
        "if (!cfg) { return null; }",
        "if (!tak.serverUrl) { tak = Object.assign({}, tak, { serverUrl: tak.takHost || 'host.docker.internal' }); }",
    ...
    },
    {
      ...
      func: [
        ...
        "// Chunk large PUTs: TAK Server 500s on a single multi-MB mission-contents PUT.",
        "// ~140 UIDs (POWER-OUTAGES) is the largest single PUT proven good in the field.",
        "// v10.1.25 R4: chunk size is ADAPTIVE per tab. TAK 5.7 rolls back a WHOLE",
        "// contents-PUT (silent NPE -> 500 code 6) if ANY uid in it has no CoT in the",
        "// repository, so log_action halves _putChunkSize on a failed batch (floor 1,",
        "// where the bad uid gets named) and doubles it back on success (cap 100).",
        "var CHUNK = flow.get('_putChunkSize') || 100;",
        "var headers = { 'accept': '*/*', 'Content-Type': 'application/json' };",
        "if (msg._missionCookie) headers.Cookie = msg._missionCookie;",
        "if (msg._missionBearer) headers.Authorization = 'Bearer ' + msg._missionBearer;",
        "if (uids.length <= CHUNK) {",
        "  msg.method = 'PUT';",
        "  msg.url = msg._putUrl;",
        "  msg.headers = headers;",
        "  msg.payload = { uids: uids };",
        "  msg._isBatchPut = true; msg._sentUids = uids;",
        "  node.warn(msg.topic + ' PUT -> ' + uids.length + ' UIDs -> ' + msg.url);",
        "  return msg;",
        "}",
        "// Large delta (e.g. cold-start seed of a nationwide feed): split into chunks",
        "// fired SEQUENTIALLY. TAK's mission-content write is delete-then-insert on",
        "// mission_uid; overlapping PUTs to the same mission race the mission_uid_pkey",
        "// constraint -> 500. The 3s spacing keeps ~one PUT in flight at a time.",
        "// setTimeout is the function node's managed timer (auto-cleared on redeploy).",
        "var url = msg._putUrl, topic = msg.topic;",
        "var total = Math.ceil(uids.length / CHUNK);",
        "node.warn(topic + ' PUT -> ' + uids.length + ' UIDs in ' + total + ' chunks of <=' + CHUNK + ' -> ' + url);",
        "var pos = 0, idx = 0;",
        "function fire() {",
        "  var chunk = uids.slice(pos, pos + CHUNK);",
        "  pos += CHUNK; idx++;",
        "  node.send({",
        "    method: 'PUT', url: url, headers: headers,",
        "    payload: { uids: chunk }, topic: topic,",
        "    _putChunk: idx, _putChunkTotal: total,",
        "    _isBatchPut: true, _sentUids: chunk",
        "  });",
        "  if (pos < uids.length) setTimeout(fire, 3000);",
        "}",
        "fire();",
        "return null;"
      ].join('\n'),
    ...
    {
      // v10.1.25 R1: rate mode, NOT a fixed delay — 'delay' shifted every DELETE by the
      // same 1s so N deletes still fired concurrently and raced TAK's delete-then-reinsert
      // mission_uid collection rewrite (duplicate mission_uid_pkey -> 500). 1 msg/sec
      // keeps at most one mission write in flight. Delay nodes are not template-synced:
      // existing engine tabs pick this up on the next config save / tab regeneration.
      id: P + 'delay_del', type: 'delay', z: FID,
      name: 'Rate limit 1/s', pauseType: 'rate', timeout: '1', timeoutUnits: 'seconds',
      rate: '1', nbRateUnits: '1', rateUnits: 'second',
      randomFirst: '1', randomLast: '5', randomUnits: 'seconds',
      drop: false, allowrate: false, outputs: 1,
    ...
    },
    {
      ...
      func: [
        ...
        "  node.warn(label);",
        "}",
        "// v10.1.25 R4: adaptive chunk feedback — TAK 5.7 rejects an entire contents-PUT",
        "// when ANY uid in it lacks a CoT in the repository; shrink toward 1 so one bad",
        "// uid can't starve the whole feed (the next poll re-PUTs whatever is missing).",
        "if (msg._isBatchPut && method === 'PUT') {",
        "  var cur = flow.get('_putChunkSize') || 100;",
        "  if (!ok) {",
        "    if (cur > 1) {",
        "      var next = Math.max(1, Math.floor(cur / 2));",
        "      flow.set('_putChunkSize', next);",
        "      node.warn(feed + ': batch PUT failed — shrinking PUT chunk ' + cur + ' -> ' + next + ' for next poll (isolating a uid TAK rejects)');",
        "    } else {",
        "      var bad = (msg._sentUids && msg._sentUids[0]) || '?';",
        "      node.warn(feed + ': uid ' + bad + ' rejected by TAK (500 at chunk=1 — likely no CoT in repository); skipped this cycle');",
        "    }",
        "  } else if (cur < 100) {",
        "    var up = Math.min(100, cur * 2);",
        "    flow.set('_putChunkSize', up);",
        "    node.warn(feed + ': PUT healthy — chunk size restored ' + cur + ' -> ' + up);",
        "  }",
        "}",
        "return msg;"
      ].join('\n'),
...
"    if (cfg.tfrUpperCase) callsign = callsign.toUpperCase();",
  ...
  "    });",
  "  }",
  "  // v10.1.25 R2: some TFRs (e.g. the standing DC security NOTAM 5/7224) are",
  "  // codeShpPrt=COMPOSITE reference-airspace entries with ZERO coordinates in the",
  "  // detail XML — nothing drawable. Say so instead of a misleading silent 'parsed'.",
  "  if (_builtCots === 0) node.warn(cfg.configName + ': ' + tfrId + ': no drawable geometry (composite/reference-airspace TFR) — skipped');",
...
const FN_TFR_RECONCILE = [
  ...
  "if (tfrUids.length === 0) {",
  "  var _cnt = flow.get('_tfrCount') || 0;",
  "  var _done = flow.get('_tfrDone') || 0;",
  "  if (_cnt > 0 && _done >= _cnt) {",
  "    node.warn(topicCfg + ' reconcile: poll parsed ' + _cnt + ' TFR(s) but 0 produced drawable geometry (composite/reference-airspace only?) — nothing to sync');",
  "  } else {",
  ...
  "",
  "// v10.1.34: the same single-poll-trust guards the ArcGIS engine got in 10.1.31.",
  "// This engine is a separate, older, duplicated implementation of the same job",
  "// (see deploy.sh 602 vs 605) and had NONE of them — not even the '0 features'",
  "// guard — so one bad poll deleted live shapes. It is materially less dangerous",
  "// than the ArcGIS case because there is no tombstone/ForceDelete machinery here:",
  "// a wrong delete stops at the mission and cannot be broadcast at every EUD or",
  "// repeated for days. Converging the two engines is the real fix and is on the",
  "// roadmap; these guards stop the bleeding without a rewrite.",
  "// v10.1.34: TIME-based, and with the magnitude guard the ArcGIS engine has. Shipped",
  "// first as 3 consecutive polls with no magnitude guard, on the reasoning that TFR is",
  "// lower risk because it has no ForceDelete machinery. Both halves were wrong, proven",
  "// on test12 2026-08-15: this engine polls every 60s, so 3 polls is 3 minutes of cover,",
  "// and a 4-minute upstream blip (17 TFRs -> 1 -> back to 17) deleted 16 of 17 live",
  "// shapes and re-added 14 three minutes later. The no-ForceDelete argument was about",
  "// blast radius to DEVICES; it ignored that the mission itself gets gutted.",
  "var TFR_MIN_ABSENCE_MS = 15 * 60000;",
  "var TFR_MASS_ABSENCE_MS = 30 * 60000;",
  "var TFR_MAG_FRACTION = 0.25;",
  "var TFR_MAG_FLOOR = 5;",
  "var tfrMissKey = '_tfrMissStreak';",
  "var tfrMisses = flow.get(tfrMissKey) || {};",
  "var nHeld = 0;",
  "var _nowTfr = Date.now();",
  "// Migrate v10.1.34-early state: counters -> epoch ms (see the ArcGIS engine).",
  "for (var _tk2 in tfrMisses) { if (!(tfrMisses[_tk2] > 1e12)) tfrMisses[_tk2] = _nowTfr; }",
  "",
  "if (Object.keys(tfrMap).length === 0) {",
  "  // 0 parsed TFRs with a mission full of them is a parse/fetch failure, not an",
  "  // empty airspace. Deleting on it would wipe the mission.",
  "  node.warn(topicCfg + ': 0 TFRs parsed this poll — skipping deletes to protect mission contents');",
  "} else {",
  "  var tfrCandidates = [];",
  "  for (var uid in existing) {",
  "    var matchesPrefix = (uid.indexOf(prefix) === 0);",
  "    if (!tfrMap[uid] && (cleanOrphans || matchesPrefix)) tfrCandidates.push(uid);",
  "  }",
  "  var nTfrExisting = Object.keys(existing).length;",
  "  var tfrMass = !oneShotPurge && tfrCandidates.length > TFR_MAG_FLOOR",
  "    && tfrCandidates.length > nTfrExisting * TFR_MAG_FRACTION;",
  "  var tfrNeedAbsent = tfrMass ? TFR_MASS_ABSENCE_MS : TFR_MIN_ABSENCE_MS;",
  "  for (var ci2 = 0; ci2 < tfrCandidates.length; ci2++) {",
  "    var uid = tfrCandidates[ci2];",
  "    {",
  "      if (!tfrMisses[uid]) tfrMisses[uid] = _nowTfr;",
  "      if (!oneShotPurge && (_nowTfr - tfrMisses[uid]) < tfrNeedAbsent) { nHeld++; continue; }",
  "      delete tfrMisses[uid];",
  "      nDel++;",
  "      node.send([null, {",
  "        method: 'DELETE',",
  ...
  "  }",
  "  if (tfrMass && nHeld > 0) {",
  "    node.warn(topicCfg + ': MASS-DELETE GUARD — this poll claims ' + tfrCandidates.length",
  "      + ' of ' + nTfrExisting + ' mission UIDs are gone; holding ' + nHeld",
  "      + ' until they have been absent ' + (TFR_MASS_ABSENCE_MS / 60000) + ' min');",
  "  }",
  "  // A UID the source still returns clears its streak; prune entries for UIDs the",
  "  // mission no longer holds so the map cannot grow unbounded.",
  "  for (var tk in tfrMap) { if (tfrMisses[tk]) delete tfrMisses[tk]; }",
  "  for (var mk in tfrMisses) { if (!existing[mk]) delete tfrMisses[mk]; }",
  "}",
  "flow.set(tfrMissKey, tfrMisses);",
  "if (oneShotPurge) {",
  "  delete forcePurge[topicCfg];",
  "  global.set('_forcePurge', forcePurge);",
  "  node.warn(topicCfg + ': one-shot purge complete, DELETED ' + nDel + ' orphan UIDs');",
...
const FN_PP_FETCH_V2 = [
  ...
  "var urlMod = global.get('nodeUrl');",
  "var crypto = global.get('nodeCrypto');",
  "if (!urlMod || !crypto) { node.warn('PulsePoint: nodeUrl/nodeCrypto missing from functionGlobalContext - redeploy Node-RED'); return null; }",
...
const FN_PP_BUILD_COT_V2 = [
  ...
  "// Stale is a HEARTBEAT DEADLINE, not a lifetime. If the next poll does not",
  "// re-publish this incident then it is no longer active and the marker should",
  "// expire by itself, rather than sitting on the map for cotStaleSec (600s by",
  "// default) after the call already cleared.",
  "//",
  "// The inject repeat is set from pollIntervalSec when the tab is built, so the",
  "// configured interval IS the heartbeat. 15s floor matches the poll gate's own",
  "// Math.max(15, ...). Stale is heartbeat + margin: one missed beat will not",
  "// flicker the marker, but a cleared call disappears within about two cycles",
  "// instead of sitting on the map for ten minutes.",
  "var effPollSec = Math.max(15, Number(cfg.pollIntervalSec) || 60);",
  "var staleSec = effPollSec + 15;",
  ...
  "  // Stale means 'valid for the next N seconds', so it is measured from NOW.",
  "  // Anchoring it to CallReceivedDateTime shipped every update to a long-running",
  "  // incident already expired -- a 40-minute structure fire arrived with a stale",
  "  // time 30 minutes in the past and ATAK dropped it (observed 2026-09-17).",
  "  var stl = new Date(Math.max(Date.now(), st.getTime()) + staleSec * 1000);",
  "  var ct = inc.PulsePointIncidentCallType || '';",
  "  var addr = (inc.FullDisplayAddress || '').trim();",
  "  // Marker label is the CALL TYPE, not the address -- operator request 2026-09-17.",
  "  // Codes are expanded where we know them; anything unrecognised falls through as",
  "  // the raw code rather than being guessed at. Address stays in remarks.",
  "  var PP_TYPES = {ME:'Medical',TC:'Traffic Collision',SF:'Structure Fire',",
  "    OF:'Outside Fire',VF:'Vehicle Fire',WF:'Wildland Fire',CF:'Commercial Fire',",
  "    RF:'Residential Fire',IFT:'Transport',PS:'Public Service',GAS:'Gas Leak',",
  "    HMR:'Hazmat',EX:'Explosion',LO:'Lockout',AA:'Auto Aid',MU:'Mutual Aid',",
  "    TCE:'Traffic Collision (Extrication)',AI:'Alarm',FA:'Fire Alarm',",
  "    MA:'Medical Alarm',WA:'Water Rescue',TE:'Technical Rescue'};",
  "  var cs = (PP_TYPES[ct] || ct || 'PulsePoint').substring(0, 100);",
